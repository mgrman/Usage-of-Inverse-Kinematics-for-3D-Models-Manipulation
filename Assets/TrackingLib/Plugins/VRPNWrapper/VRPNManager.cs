...
{   
    ...
    /// <summary>
    /// Hack for bug when calling Loop or Starting native server crashes Unity after Stoping it once.
    /// This only stops Unity from crashing, doesnt fix it.
    /// </summary>
    private bool _wasDisabledOnce = false;

	void Start()
	{
#if UNITY_64 || UNITY_EDITOR_64

        //If you have supplied 64bit version of this library, then please remove this check.
        Debug.LogError("VRPNWrapper is only availabe in x86 (32bit) version! You can build x86 desktop version or download 32bit version of Unity Editor to use it.");
        return;
#endif
        ...
        //var dllPath = Path.Combine(Path.Combine(Path.Combine(Application.dataPath, "Plugins"), "VRPNWrapper"), "VRPNWrapper.dll");
        //print(dllPath);
        //LoadNativeDll(dllPath);
		...
		try
		{
			...
			for (int index=0; index<ConfigLines.Length; index++) 
			{
				if (ConfigLines[index].StartsWith("vrpn_"))
				{
					...
					{
						if (tracker.enabled && splitResult[0] == tracker.TrackerType.ToString() && splitResult[1] == tracker.TrackerName)
						{
